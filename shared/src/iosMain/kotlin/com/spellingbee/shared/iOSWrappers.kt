...
class IOSGameManager {
    ...
    var onUserProgressChanged: ((Map<String, UserGameProgress>) -> Unit)? = null
    ...
    init {
        ...
        gameManager
                .isLoading
                .onEach { loading -> onIsLoadingChanged?.invoke(loading) }
                .launchIn(scope)
        ...
        gameManager
                .authManager
                .currentUser
                .onEach { user -> onAuthUserChanged?.invoke(user) }
                .launchIn(scope)
        gameManager
                .authManager
                .isLoading
                .onEach { loading -> onAuthLoadingChanged?.invoke(loading) }
                .launchIn(scope)
        gameManager
                .authManager
                .error
                .onEach { error -> onAuthErrorChanged?.invoke(error) }
                .launchIn(scope)
        ...
        gameManager
                .storageManager
                .isUploading
                .onEach { uploading -> onStorageLoadingChanged?.invoke(uploading) }
                .launchIn(scope)
        gameManager
                .storageManager
                .error
                .onEach { error -> onStorageErrorChanged?.invoke(error) }
                .launchIn(scope)
        // User progress callbacks
        gameManager
                .userProgress
                .onEach { progress -> onUserProgressChanged?.invoke(progress) }
                .launchIn(scope)
    }
    ...
    fun register(
            ...
            email: String,
            password: String,
            completion: (SpellGameUser?, String?) -> Unit
    ) {
        scope.launch {
    ...
    fun loadUserProgress(completion: () -> Unit) {
        scope.launch {
            gameManager.loadUserProgress()
            withContext(Dispatchers.Main) { completion() }
        }
    }
    ...
    fun updateUserProgress(
        ...
        scope.launch {
            val result =
                    gameManager.updateUserProgress(
                            gameId,
                            userId,
                            wordIndex,
    ...
    fun getUserProgress(userId: String, gameId: String): UserGameProgress? {
        return gameManager.getUserProgress(userId, gameId)
    }
}

// Helper function to generate UserGameProgress ID
fun generateUserProgressId(userId: String, gameId: String): String {
    return UserGameProgress.generateId(userId, gameId)
}
